...
//			options.TableNames = arguments.Skip(2).ToList();
		...
		{
		    var connectionString = "Server=.\\SQLEXPRESS;Database=ERA_Certificate_Info;Trusted_Connection=True";// ConfigurationManager.ConnectionStrings["Origin"].ToString(); // GetConnectionString(options.Server, options.Database, options.UseSqlServerAuthenication, options.Username, options.Password);
...
//                var numberPrefix = DateTime.Now.ToString("yyyy-MM-dd-HHmm.");
                var numberPrefix = "0117_"; //@Prath - something like this should work well, for our purposes, just manually pick a decent number and update here before running the tool
                var fileNamePrefix = numberPrefix + iFile.ToString("D2");
                iFile++;
